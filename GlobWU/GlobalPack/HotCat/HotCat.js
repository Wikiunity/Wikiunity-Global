...
 
 ...
 ,category_regexp    : '[Cc][Aa][Tt][Ee][Gg][Oo][Rr][Yy]'
   ...
   // Chinese Wikipedia, use '[Cc][Aa][Tt][Ee][Gg][Oo][Rr][Yy]|??|??'. Note that namespaces are case-
   // insensitive!
...
 
      ...
      // FF3.6: add the input field first, then the two absolutely positioned elements. Otherwise,
      // FF3.6 may leave the suggestions and the selector at the right edge of the screen if
      // display of the input field causes a re-layout moving the form to the front of the
      // next line.
...
 
      ...
      if (textPos.y < maxListHeight + offset + 1) {
        ...
        if (haveEngine) this.engineSelector.style.top = this.text.offsetHeight + 'px';
      } else {
        nt = - listh - offset - 1;
        if (haveEngine) this.engineSelector.style.top = - (offset + 1) + 'px';
      }
